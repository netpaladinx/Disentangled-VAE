...
import utils as U
from model_benchmark import get_model_obj
...
parser.add_argument('-m', '--model_type', type=str, default='dip_vae_i')
...
def train(args):
    ...
    args.tag = '{},{},{},{},{},exp{}'.format(args.name, args.version, args.dataset, args.model_type,
                                             hp_tags[args.model_type], args.experiment)
    ...
    model = get_model_obj(args.model_type, args).to(device)
    ...
    for batch in train_loader:
        ...
        recon_x, z_mean, z_logvar, _ = model(x)

        if args.model_type == 'annealed_vae':
            losses = model.loss(x, recon_x, z_mean, z_logvar, step, recon_type=args.recon_type)
        else:
            losses = model.loss(x, recon_x, z_mean, z_logvar, recon_type=args.recon_type)

        if args.model_type == 'factor_vae':
            d_loss = model.discriminate(z_mean, z_logvar)
        ...
        elif args.model_type == 'annealed_vae':
            loss, neg_elbo, recon_loss, kl_loss = losses
            loss_avg, neg_elbo_avg, recon_loss_avg, kl_loss_avg = \
                U.moving_average(moving, (loss.item(), neg_elbo.item(), recon_loss.item(), kl_loss.item()),
                                 (loss_avg, neg_elbo_avg, recon_loss_avg, kl_loss_avg))
        ...
        elif args.model_type in ['dip_vae_i', 'dip_vae_ii']:
        ...
        elif args.model_type == 'beta_tc_vae':
            loss, neg_elbo, recon_loss, kl_loss, tc_loss = losses
            loss_avg, neg_elbo_avg, recon_loss_avg, kl_loss_avg, tc_loss_avg = \
                U.moving_average(moving, (loss.item(), neg_elbo.item(), recon_loss.item(), kl_loss.item(), tc_loss.item()),
                                 (loss_avg, neg_elbo_avg, recon_loss_avg, kl_loss_avg, tc_loss_avg))
        ...
        if step % args.print_freq == 0:
            print('[{}, {}, {}][Step {:d}] {}'.format(
                args.model_type, args.dataset, hp_tags[args.model_type], step, loss_info))
